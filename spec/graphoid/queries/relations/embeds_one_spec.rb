...
describe "QueryEmbedsOne", :type => :request do
  ...
  let!(:v0) { Value.create!(text: 'ac', account: a0) }
  let!(:v1) { Value.create!(text: 'bb', account: a1) }
  let!(:v2) { Value.create!(text: 'bb', account: a2) }
  ...
  describe "filtering with conditions in embeds_one relations" do
    it "filters properly" do
      ...
      expect(subject[1]["value"]["id"]).to eq v2.id.to_s
    end
